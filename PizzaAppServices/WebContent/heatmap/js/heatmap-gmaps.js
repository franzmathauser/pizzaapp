...
HeatmapOverlay.prototype.draw = function() {
	...
	var overlayProjection = this.getProjection(), currentBounds = this.map
			.getBounds();
	...
	if (this.latlngs.length > 0) {
		...
		var len = this.latlngs.length, projection = this.getProjection();
		d = {
...
HeatmapOverlay.prototype.setDataSet = function(data) {
	...
	var d = data.data, dlen = d.length, projection = this.getProjection();
...
HeatmapOverlay.prototype.addDataPoint = function(lat, lng, count) {
	...
	var projection = this.getProjection(), latlng = new google.maps.LatLng(lat,
			lng), point = this.pixelTransform(projection
